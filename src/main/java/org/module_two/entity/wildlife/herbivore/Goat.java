...
public class Goat extends Herbivore {     //Коза
    ...
    public Goat(double maxWeight, int movementSpeed, double kgForSatiety, int hp) {
    ...
    public void reproduce(int indexWLocation, int indexHLocation) {
        ...
        if(location.getCountAnimalSpecies(GOAT) < (int) (Island.gameInputStartData.getCharacteristic(GOAT
                , MAX_COUNT_ANIMAL_OR_PLANT_INDEX_TABLE) * 0.9)){
            location.addAnimal(GOAT, new Goat(
    ...
    public void fillTableProbabilityEat(){
        ...
        probabilityEat.put(PLANT, Island.gameInputStartData.getProbabilityCurrentAnimalEatFood(GOAT, PLANT) / 100.0);
    }
}
