...
public class Horse extends Herbivore {
    ...
    public Horse(double maxWeight, int movementSpeed, double kgForSatiety, int hp) {
    ...
    public void reproduce(int indexWLocation, int indexHLocation) {
        ...
        if(location.getCountAnimalSpecies(HORSE) < (int) (Island.gameInputStartData.getCharacteristic(HORSE
                , MAX_COUNT_ANIMAL_OR_PLANT_INDEX_TABLE) * 0.9)){
            location.addAnimal(HORSE, new Horse(
    ...
    public void fillTableProbabilityEat(){
        ...
        probabilityEat.put(PLANT, Island.gameInputStartData.getProbabilityCurrentAnimalEatFood(HORSE, PLANT) / 100.0);
    }
}
