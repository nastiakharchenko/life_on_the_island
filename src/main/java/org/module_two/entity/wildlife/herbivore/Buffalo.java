...
public class Buffalo extends Herbivore {  //Буйвол
    ...
    public Buffalo(double maxWeight, int movementSpeed, double kgForSatiety, int hp) {
    ...
    public void reproduce(int indexWLocation, int indexHLocation) {
        ...
        if(location.getCountAnimalSpecies(BUFFALO) < (int) (Island.gameInputStartData.getCharacteristic(BUFFALO
                , MAX_COUNT_ANIMAL_OR_PLANT_INDEX_TABLE) * 0.9)){
            location.addAnimal(BUFFALO, new Buffalo(
    ...
    public void fillTableProbabilityEat(){
        ...
        probabilityEat.put(PLANT, Island.gameInputStartData.getProbabilityCurrentAnimalEatFood(BUFFALO, PLANT) / 100.0);
    }
}
