...
public class Mouse extends Herbivore {
    ...
    public Mouse(double maxWeight, int movementSpeed, double kgForSatiety, int hp) {
    ...
    public void reproduce(int indexWLocation, int indexHLocation) {
        ...
        if(location.getCountAnimalSpecies(MOUSE) < (int) (Island.gameInputStartData.getCharacteristic(MOUSE
                , MAX_COUNT_ANIMAL_OR_PLANT_INDEX_TABLE) * 0.9)){
            location.addAnimal(MOUSE, new Mouse(
    ...
    public void fillTableProbabilityEat(){
        ...
        probabilityEat.put(CATERPILLAR, Island.gameInputStartData.getProbabilityCurrentAnimalEatFood(MOUSE, CATERPILLAR) / 100.0);
        probabilityEat.put(PLANT, Island.gameInputStartData.getProbabilityCurrentAnimalEatFood(MOUSE, PLANT) / 100.0);
    }
}
